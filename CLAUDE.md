...
7. **Test after every model change.** Run `npm test` before committing any change to `src/model/` or `src/config/`. The suite (806 tests) includes a **golden master** (`src/model/__tests__/golden-master.test.js`) that locks every headline number at the default state — if it fails, a model change moved a value. Update the locked values only when the change was intended.
...
- **Duration-event lost-income modeling + cushion-based verdict (2026-07-13, branch
  ...
  **Solvers tab removed** by owner decision (Dials + Events cover the job; calcAffordabilityMax
  ...
  Dials). 763 → **806 tests** (net: −2 Solvers removal, +~32 income channel incl. a big-trip
...
- `npm test` — run model + formatter + render-smoke tests (806 tests)
