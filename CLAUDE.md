...
7. **Test after every model change.** Run `npm test` before committing any change to `src/model/` or `src/config/`. The suite (574 tests) includes a **golden master** (`src/model/__tests__/golden-master.test.js`) that locks every headline number at the default state — if it fails, a model change moved a value. Update the locked values only when the change was intended.
...
- Feature backlog: `feature-tracker.html` (120 items, 54 done, 66 planned)
...
- **Level 3 — Control begins: setter bundles + My details (2026-06-26, branch
  ...
  560 → **574** tests, lint clean, build OK.
...
- `npm test` — run model + formatter + render-smoke tests (574 tests)
