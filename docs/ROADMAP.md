...
14. **Tests gate the wiring.** Every new screen ships a render-smoke test at golden-master defaults; every displayed number that has a locked value gets a wiring assertion; hardcoded UI tables (`RETIRE_JUMPS`, `LIFE_EVENTS`) get value-locks so silent edits are caught. *(Renamed 2026-07-12: the locked "Scenarios" preset table `SCENARIOS` was retired — its age-only cards became `RETIRE_JUMPS`, the Dials quick-jump chips; see the dated note under SP-5 below.)* (Detailed in the Testing strategy section — listed here because it is the enforcement arm of principles 6–12.)
...
- **SP-3 — Uncertainty is a lens, not a screen.** Monte Carlo (#38) ships as real percentile data behind the arc's band view (**renamed "Range"** — the current "Scenarios" name collides with Ideas' scenario cards), with the success % as the Range view caption, a driver line in the on-track pill popover, and a low-odds signal that deep-links to the working-longer card (#55). Historical stress (#39, premium) is a "History" mode of the same view (named paths: 1966, 2000, 2008). **No second glance-level verdict on Plan** — one pill; long-term the pill's verdict itself becomes confidence-aware *in the model*. **Owner note (verbatim, binding):** Monte Carlo charts are a feature users often like seeing; confidence levels must be visually available somewhere (the Range view is that home). After the lens ships and is tested in practice, revisit whether the recommended route is still best — alternatives should be explored. **Status note (2026-07-28, Session B):** the lens now walks the per-account engine instead of the older blended walk — a materially more accurate version of "shipped and tested in practice." This is flagged as a natural trigger point for the revisit above; the revisit itself (whether "lens not screen" is still the best route) is an owner call and was not actioned in Session B.
  **Dated correction (2026-07-12):** the naming collision this rename cites is now moot — Ideas' "Scenarios" mode was retired the same day (see SP-5's dated note). The "Range" rename for the ArcGraph band view is unaffected and stays planned (out of scope for that change); it's just no longer urgent for the collision reason originally given.
...
- **SP-5 — Surface governance.** **Numbers stays ≤ 6 tabs forever** — extend an existing tab before adding one (state exemptions #52 → a "Where you'll retire" section of Taxes; NIIT #45 → a Taxes warning row + a surtax line in the conversion flow's impact panel; withdrawal sequencing #47 → per-account draw columns in Year-by-year + upgrades the Withdrawal-order card to Apply-with-preview). **Ideas = arc + verdict badge (#85) on top + ONE segmented mode control: Dials · Events · Scenarios · Solvers** (affordability + #82 recurring-spend + #83 required-contribution = three questions in one Solvers panel; #84 upgrades the retirement-age dial to a free slider) — never stacked panels. **My details grows only by collapsed topic cards** (5 at L3 → ~9 at end state: adds Property & rentals, Business & partnerships, Family & estate, Spouse & household, Income & expense timeline) — closed cards are one summary line, so 9 cards stay calmer than Classic's 110 visible inputs.
  **Dated correction (2026-07-12, owner decision):** the locked "Scenarios" segment named above was retired — its 3 age-only cards became Dials quick-jump chips (`RETIRE_JUMPS`) and its "Big trip" card folded into Events as a normal editable pill. Ideas is **Dials · Events** today; the future shape when Solvers ships (WI-5.4/#105) is **Dials · Events · Solvers** (3 modes), not the 4 named above. See `docs/BUGS.md` BUG-44 addendum and `CLAUDE.md`'s dated status entry for the full change.
  **Dated correction (2026-07-13, owner decision):** the Solvers tab and its corresponding `AffordabilityPanel.jsx` were removed — owner decided Dials + Events cover the affordability job and the Solvers-mode addition was redundant. `calcAffordabilityMax` is retained in the model for Classic's `WhatIfPanel` Max Affordable mode (unchanged). Ideas is now **Dials · Events** (2 modes), not 3. The future roadmap for SP-5 above references "Dials · Events · Solvers" and "affordability + #82 recurring-spend + #83 required-contribution in one Solvers panel" — that planning is deferred pending a future affordability-feature reevaluation (this is a scope trim, not a change in direction: the technical foundation is complete; product direction is adjustable).
...
| 84 | Free retirement-age slider | — | Ideas → Dials (upgrade) | — | Free (#84's slider upgrade shipped early with #122 — Ideas' Dials are live sliders now.) |
...
- Layout: Ideas already has a segmented mode mechanism (`mode` state + mode buttons) — both additions land as **new modes of that one control** (pre-adopting SP-5's naming), never as stacked panels. *(Dated correction, 2026-07-12: SP-5's naming is now Dials · Events · Solvers — the Scenarios segment named here was retired; see SP-5's dated note.)*
...
**Done when:** an event added in Horizon appears in Classic's panel and moves the arc; affordability result equals Classic `WhatIfPanel`'s for identical inputs; one mode visible at a time (SP-5). **Partially pulled forward 2026-07-11 (#122):** the commitPlan→ApplyPreviewModal routing residual is DONE (Plan's Apply and Ideas' "Apply to my plan" both route through ApplyPreviewModal via applyPlanLevers); the events editor and affordability/solvers modes remain open. Ideas' segmented mode control (SP-5's shape) and live dial sliders (#84's free-slider upgrade for the retire-age dial) also shipped early.
...
**Dated correction (2026-07-12):** merged into the arc-event-placement branch alongside the
Scenarios removal (see SP-5's dated note above). The `"events"` mode / `eventsView` / `commitPlan`
sites described above were retired the same day — `EventsEditorPanel.jsx` deleted, superseded by
#121's sheet-first `LifeEventSheet` flow (owner had separately rejected the raw-editor pattern);
`planCommit`/`buildScenarioCommitSite` retired with QuickTunePanel, superseded by #122's
`applyPlanLevers`. The `"solvers"` mode / `AffordabilityPanel.jsx` / `affordView` — a genuinely
independent addition with no overlap — was kept as Ideas' 3rd segment (Dials · Events · Solvers).
`surplusApplySite`/`surplusView`/`withdrawalView`/`megaView` (WI-3.7, above) are unaffected — common
ancestry both branches already shared. See `docs/ARCHITECTURE.md`'s `affordView` section and
`docs/BUGS.md`'s BUG-44 addendum for the full detail.
Golden master untouched.
**Dated correction (2026-07-13):** the Solvers tab and its corresponding `AffordabilityPanel.jsx`
were removed by owner decision — Dials + Events cover the affordability job adequately, and the
addition was redundant with the existing sheets/controls. `calcAffordabilityMax` is retained in the
model for Classic's `WhatIfPanel` Max Affordable mode (unchanged). Ideas is now **Dials · Events** (2
modes), not 3. The `affordView` bundle was removed from `horizonProps`. Affinity testing (BUG-63/68
desktop-mobile age-input clamp/bound-sync) was completed before removal and was shipped anyway (the
fixes apply to any future solver surface). See `docs/BUGS.md` BUG-72/73/74 close-out and
`CLAUDE.md`'s dated status entry.
...
**Actions:** segmented mode control **Dials · Events · Solvers** *(dated correction, 2026-07-12: was planned as 4 modes including a locked "Scenarios" segment; the owner retired that segment the same day — its age-only cards became Dials quick-jump chips, its "Big trip" card folded into Events — so the future shape here is 3 modes, not 4)*; verdict badge #85 in the header (and inside ApplyPreviewModal); Solvers panel unifying affordability (WI-3.8) + #82 recurring-spend + #83 required-contribution as three questions of one panel; #84 free retirement-age slider replacing the ±2-year presets; #40 My scenarios shelf + compare sub-view (premium; mobile stacked A/B).
...
| What-if overlay: scenario delta mode | Ideas (shipped: dials) | — |
