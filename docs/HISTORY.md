...
This file holds the full, session-by-session build history that used to live in
`CLAUDE.md`'s `## Status` section. It was split out on 2026-07-27 (part of the PR #62
review battery's documentation-debt follow-through — see `docs/BUGS.md` → "PR #62
review battery" and the forward-compat audit's recommendation it responds to) once
`CLAUDE.md` grew large enough that a session could no longer hold its full context.

**`CLAUDE.md`'s `## Status` section keeps only the most recent sessions** (roughly the
current active work arc — currently the #30 spouse-engine stabilization saga starting
at BUG-82). Everything older lives here, in the same chronological order, verbatim
(no content was edited or summarized during the move — this is a relocation, not a
rewrite). If you need context on a specific feature, PR, or bug from before BUG-82,
search this file; `docs/BUGS.md` remains the authoritative bug record regardless of
which history file a build note lives in.
